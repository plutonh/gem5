...
sys.stdout = open('/home/jonghyeon/gem5/DBE/ExperimentResults.txt', 'w')
...
data_random = open("/home/jonghyeon/gem5/m5out/binary_out.txt","r")
...
def doExperiment(_file,_num):
    ...
    const_NRZ = 15 * 3.3 * 10e-3 / 8
    const_PAM4 = 15 * 3.3 * 10e-3 / 16
    ...
    for i in range(len(lines)):
        ...
        if sum(data_NRZ) <= 4:
            ...
            if example_1 < 3:
                ...
                print('switching power: ', chargeCount, ' * ', const_NRZ, ' = ', chargeCount * const_NRZ)
        elif example_1 < 3:
            print('#1 NRZ-DBI')
        ...
        if 3 * DBI_Flag.Flags[0] + DBI_Flag.Flags[1] - DBI_Flag.Flags[2] - 3 * DBI_Flag.Flags[3] > 0:   # before power > after power => inversion
            ...
            if example_2 < 3:
                print('#2 PAM4-DBI')
                print('charging 2: ', '(', calCharging(3,0), ' * ', DBI_Flag.Flags[0], ' + ', calCharging(2,1), ' * ', DBI_Flag.Flags[1], ')', ' * ', const_PAM4, ' = ', (calCharging(3,0) * DBI_Flag.Flags[0] + calCharging(2,1) * DBI_Flag.Flags[1]) * const_PAM4)
        ...
        if example_2 < 3:
            ...
            print('charging 3: ', calCharging(3,maxIndex),' * ', DBI_Flag.Flags[3], ' * ', const, ' = ', calCharging(3,maxIndex) * DBI_Flag.Flags[3] * const_PAM4)
        ...
        for i in range(len(cList)):
            ...
            if example_2 < 3:
                ...
                print('charging #', i, ': ', calCharging(cList[i][1], 0), ' * ', DBI_Flag.Flags[cList[i][0]], ' * ', const_PAM4, ' = ', calCharging(cList[i][1], 0) * DBI_Flag.Flags[cList[i][0]] * const_PAM4)
        _count = 1
        # print('\n\n\n\n\n')
        if example_2 < 3:
    ...
    print("NRZ-DBI Power = ", charging_1 * const, "W     encoded ", eCount_1, ' times')
    print("PAM4-DBI Power = ", charging_2 * const, "W     encoded ", eCount_2, ' times')
